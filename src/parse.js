...
function parse(setResult, warn, state) {
    ...
    if (number.match(/[^0-9]/)) {
        setResult("Error: illegal number");
        return;
    }
    if (number.length > 24) {
        setResult("Error: number too large.  Limit is 10**24.");
        return;
    }
    ...
    do {
        ...
        if (number.length < 3 || tripVal) {  // Handle special case of 0
            let trp = new Triplet(tripVal, index, context, state.noun, warn);
