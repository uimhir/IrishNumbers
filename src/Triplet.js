...
class Triplet {
    constructor(n, index, context, noun, warn) {
        this.n       = n;
        this.index   = index;
    ...
    render(lowest, simple, highest) {
        ...
        let peoplePrefix = [null, null, "beirt",
            "triúr", "ceathrar", "cúigear",
            "seisear", "seachtar", "ochtar",
            "naonúr", "deichniúr", null, "dháréag"];
            ...
            switch (context) {
                ...
                case "objects":
                    switch(n){
                        ...
                        case 1: 
                            if (index == 0 && highest) {
                                return "aon" + sNoun(lenite(noun)) + sNoun("amháin");
                            } else {
                                if (lowest) {
                                    if (index > 0) {
                                        return sNoun(noun);
                                ...
                                } else {  // This will be the current tripletUnits.
                                    return sNoun(noun); 
                                }
                ...
                case "ordinal":
                    switch(n) {
                        case 0:
                            here.warn("Non standard construction: "+ordinalPrefix[0]+" for zeroth");
            ...
            if (n < 10) {
                ...
                switch (context) {
                    ...
                    case "ordinal":
                        ...
                        // If we aren't following a hundred, then the 'is' is not included.
                    ...
                    default:
                        ...
                        if (simple  && !(lowest && highest && index>0)) {
                            let res = "";
                            if (bigger) {
                                res = "a " + hPrefix(basePrefix[n-10]);
                            } else {
                            ...
                            // Note that nouns with plurals suppress lenition of a following deag
                            // even when the special plural is not used (i.e., n=1 or 2), so we
                            // use n=3 rather than the actual value since specialPlural may not
                            // return anything for 1 or 2.
                            let specPlur = specialPlural(noun, 3);
                            let finalChar = noun.substring(noun.length-1);
                            let isVowel = vowels[finalChar];
                            
                            if (noun.toLowerCase() == "euro") {
                                here.warn("The noun euro does not cause lenition of following déag")
                                isVowel = false;                                
                            }
                           
                            if ((!specPlur && isVowel)  || (noun == "míle"  && index == 1)) {
                                res += " dhéag";
                            } else {
                                res += " déag";
                            }
        ...
        function threeDigits() {
            ...
            if (n % 100 == 0) {
                ...
                let sep = " ";
                // We want 150 x to render as "cead is cuig x", but there is
                // an example that suggests this should not extend to higher values...
                if (context == "ordinal"  || (context == "objects"  && (!simple  || (n%10 == 0  && index == 0 && highest) ) ) ) {                    
                    if (n < 20 || n%10 == 0) {
                        sep = " is ";
                    }                    
                }
        ...
        function leniteOrUru(noun, n, vowel) {
            ...
            let lc = noun.toLowerCase();
            if (lc == "euro"  || lc == "cent") {
                if (n > 6) {
                    here.warn("No eclipsis for "+noun+" following 7-10");
                } else if (n > 1 && lc == "cent") {
                    here.warn("No lenition of "+noun+" following 1-6");
                }
                return noun;
            }
        ...
        function uru(noun, vowel) {
            ...
            if (noun == "billiún") {
                ...
                // hack, dontradicted by the standard.
                here.warn("Suppressed required uru of billiún");
                return noun;
            }
            let init = noun.substring(0,1);
            if (consanUru[init]) {
        ...
        function specialPlural(noun, n) {
            ...
            if ( (n%10 == 2) && specials2[noun]) {
                ...
                if (specials3[noun]) {
                    if (ambig[noun]) {
                        here.warn("The noun "+noun+" has a numeric plural form in only some meanings.  Plural form assumed.");
                    }
        ...
        function genitivePlural(noun) {
            ...
            if (!val) {
                here.warn("Noun "+noun+" not recognized in context which requires genitive plural");
                return noun;
            }
            if (!val.gp) {
                here.warn("No genitive plural form found for "+noun+".  Using nominative singular in context which requires genitive plural");
                return noun;
            }  else {
                return val.gp;
            }
        ...
        function gender(noun) {
            ...
            let word = nouns[noun];
            if (word == null) {
                here.warn("Attempt to use unknown noun, "+noun+", in context where gender is required.  Assuming masculine.");
                return "m";
            }
            if (!word.gender) {
                here.warn("No gender defined for noun.  Assuming masculine.");
                return "m";
            }
            return word.gender;
        }        
